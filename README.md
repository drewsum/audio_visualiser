...
| Job | What it does |
|---|---|
| Repo hygiene | Fails on committed backups, lock files, caches or generated outputs, and on files not saved by KiCad 10.0. |
...
Per-project settings live in `hdw/ci-config.json`. Each of `erc`, `drc` and `libraries` is `enforce` (fail the build), `report` (annotate only) or `off`. Personal libraries that aren't in this repo are listed under `external_libraries` and cloned by CI under the same nicknames.
